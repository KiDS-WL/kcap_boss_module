!module containing the quadpack Fortran subroutines for numerical 
!integration by Robert Piessens, Elise deDoncker-Kapenga, Christian Ueberhuber, 
!David Kahaner.
!Reference:
...
!  Parameters:
!
!    Input, external real(kind=dp) F, the name of the function routine, of the form
!      function f ( x )
!      real(kind=dp) f
!      real(kind=dp) x
...
!
!    Input, real(kind=dp) EPSABS, EPSREL, the absolute and relative accuracy requested.
!
...
!
!    Output, real(kind=dp) ABSERR, an estimate of || I - RESULT ||.
!
...
!
  ...
  real(kind=dp) a
  real(kind=dp) abserr
  real(kind=dp) alist(limit)
  real(kind=dp) b
  real(kind=dp) blist(limit)
  real(kind=dp) elist(limit)
  real(kind=dp) epsabs
  real(kind=dp) epsrel
...
!    6, the input is invalid, because EPSABS < 0 and EPSREL < 0.
!
!    Workspace, real(kind=dp) ALIST(LIMIT), BLIST(LIMIT), contains in entries 1 
!    through LAST the left and right ends of the partition subintervals.
!
!    Workspace, real(kind=dp) RLIST(LIMIT), contains in entries 1 through LAST
!    the integral approximations on the subintervals.
!
!    Workspace, real(kind=dp) ELIST(LIMIT), contains in entries 1 through LAST
...
!
      if ( max ( abs ( a1 ), abs ( b2 ) ) <= ( 1.0_dp + c * 1.0e+03 * &
...
!    which evaluates the integrand function.
!
!    Input, real(kind=dp) BOUND, the value of the finite endpoint of the integration
...
!
  ...
  real(kind=dp) reseps
  real(kind=dp) result
  real(kind=dp) res3la(3)
  real(kind=dp) rlist(limit)
  real(kind=dp) rlist2(52)
  real(kind=dp) small
!
!  Test on validity of parameters.
!
...
!
  ...
  if ( ( 1.0_dp - 5.0e+01 * epsilon ( defabs ) ) * defabs <= dres ) then
    ksgn = 1
  else
    ksgn = -1
  end if
...
!
    if ( max ( abs(a1), abs(b2) ) <= (1.0_dp + 1.0e+03 * epsilon ( a1 ) ) * &
...
!
  ...
  if ( result /= 0.0_dp .and. area /= 0.0_dp) then
    go to 105
  end if
...
!
!    Input/output, real(kind=dp) POINTS(NPTS2), contains the user provided interior
...
!
  ...
  if ( npts2 < 2 ) then
    ier = 6
    return
  else if ( limit <= npts .or. ( epsabs < 0.0_dp .and. &
    epsrel < 0.0_dp) ) then
    ier = 6
    return
  end if
...
!
  ...
  do i = 1, nint
    ...
    if ( error1 == resa .and. error1 /= 0.0_dp ) then
      ndin(i) = 1
    end if
...
!
  ...
  if ( dres >= ( 1.0_dp - 0.5E+00 * epsilon ( resabs ) ) * resabs ) then
    ksgn = 1
  else
    ksgn = -1
  end if
...
!
  ...
  if ( dres >= (1.0_dp-5.0e+01* epsilon ( defabs ) ) * defabs ) then
    ksgn = 1
  else
    ksgn = -1
  end if
...
!    Input, real(kind=dp) A, B, the limits of integration.
!
!    Input, real(kind=dp) C, a parameter in the weight function, which must
...
!    Input, real(kind=dp) A, B, the limits of integration.
!
!    Input, real(kind=dp) C, a parameter in the weight function, which cannot be
!    equal to A or B.
!
!    Input, real(kind=dp) EPSABS, EPSREL, the absolute and relative accuracy requested.
!
...
!
  ...
  if ( c == a  ) then
    ...
    ier = 6
    return
  else if ( epsabs < 0.0_dp .and. epsrel < 0.0_dp ) then
    ier = 6
    return
  end if
!
...
!
!    Input, real(kind=dp) OMEGA, the parameter in the weight function.
!
!    Input, integer INTEGR, indicates which weight functions is used
!    = 1, w(x) = cos(omega*x)
!    = 2, w(x) = sin(omega*x)
!
!    Input, real(kind=dp) EPSABS, the absolute accuracy requested.
!
!    Output, real(kind=dp) RESULT, the estimated value of the integral.
!
!    Output, real(kind=dp) ABSERR, an estimate of || I - RESULT ||.
...
!
!            alist, blist, rlist, elist - real(kind=dp)
...
! real(kind=dp) dla
  ...
  integer numrl2
  real(kind=dp) omega
  real(kind=dp), parameter :: p = 0.9E+00
  real(kind=dp), parameter :: pi = 3.1415926535897932E+00
  real(kind=dp) p1
  real(kind=dp) psum(52)
  real(kind=dp) reseps
  real(kind=dp) result
  real(kind=dp) res3la(3)
  real(kind=dp) rlist(limit)
...
!
  ...
  if ( omega == 0.0_dp ) then
    ...
    if ( integr == 1 ) then
      call qagi ( f, 0.0_dp, 1, epsabs, 0.0_dp, result, abserr, neval, ier )
    else
      result = 0.0E+00
      abserr = 0.0E+00
      neval = 0
...
!    Input, real(kind=dp) A, B, the limits of integration.
!
!    Input, real(kind=dp) ALFA, BETA, parameters used in the weight function.
...
!    Input, real(kind=dp) A, B, the limits of integration.
!
!    Input, real(kind=dp) C, the parameter in the weight function.
!
!    Output, real(kind=dp) RESULT, the estimated value of the integral.
...
!
  ...
  real(kind=dp) cc
  real(kind=dp) centr
  real(kind=dp) cheb12(13)
  real(kind=dp) cheb24(25)
  real(kind=dp), external :: f
  real(kind=dp) fval(25)
  real(kind=dp) hlgth
  integer i
  integer isym
  integer k 
  ...
  real(kind=dp) res24
  real(kind=dp) u
  real(kind=dp), parameter, dimension ( 11 ) :: x = (/ &
...
!
  ...
  do k = 3, 13
    ...
    ak22 = ( k - 2 ) * ( k - 2 )
    if ( ( k / 2 ) * 2 == k ) then
      amom2 = amom2 - 4.0_dp / ( ak22 - 1.0_dp )
    end if
...
!    Output, integer NEVAL, the number of times the integral was evaluated.
!
!    Output, real(kind=dp) RESABS, approximation to the integral J.
!
!    Output, real(kind=dp) RESASC, approximation to the integral of abs(F-I/(B-A)).
!
!         on entry and return
!           momcom - integer
...
!
  ...
  v(3) = (3.2e+01*(par2-1.2e+01)*cospar+(2.0_dp* &
...
!
  ...
  ass = parint * sinpar
  asap = (((((2.10e+02*par2-1.0_dp)*cospar-(1.05e+02*par2 &
     -6.3e+01)*ass)/an2-(1.0_dp-1.5e+01*par2)*cospar &
     +1.5e+01*ass)/an2-cospar+3.0_dp*ass)/an2-cospar)/an2
...
!
  ...
  v(2) = (1.8e+01-4.8e+01/par2)*sinpar/par2 &
     +(-2.0_dp+4.8e+01/par2)*cospar/parint
  ...
  if ( abs(parint) <= 2.4e+01 ) then
    ...
    do k = 3, 12
      ...
      chebmo(m,2*k) = -sinpar/(an*(2.0_dp*an-2.0_dp)) &
                 -2.5e-01*parint*(v(k+1)/an-v(k)/(an-1.0_dp))
    end do
!
!  Compute the Chebyshev moments by means of forward recursion.
!
...
!    Input, real(kind=dp) A, B, the limits of integration.
!
!    Input, real(kind=dp) BL, BR, the lower and upper limits of integration.
!    A <= BL < BR <= B.
!
!    Input, real(kind=dp) ALFA, BETA, parameters in the weight function.
!
!    Input, real(kind=dp) RI(25), RJ(25), RG(25), RH(25), modified Chebyshev moments 
!    for the application of the generalized Clenshaw-Curtis method,
!    computed in QMOMO.
!
!    Output, real(kind=dp) RESULT, the estimated value of the integral, computed by 
...
!    Output, real(kind=dp) ABSERR, an estimate of || I - RESULT ||.
!
!    Output, real(kind=dp) RESASC, approximation to the integral of abs(F*W-I/(B-A)).
!
...
!
  ...
  if ( bl == a .and. (alfa /= 0.0_dp .or. integr == 2 .or. integr == 4)) then
    go to 10
  end if
...
!  Parameters:
!
!    Input, real(kind=dp) X(11), contains the values of COS(K*PI/24), for K = 1 to 11.
!
!    Input/output, real(kind=dp) FVAL(25), the function values at the points
...
!
!    Output, real(kind=dp) CHEB12(13), the Chebyshev coefficients for degree 12.
!
!    Output, real(kind=dp) CHEB24(25), the Chebyshev coefficients for degree 24.
!
  implicit none
...
!
!    Input/output, real(kind=dp) EPSTAB(52), the two lower diagonals of the triangular
!    epsilon table.  The elements are numbered starting at the right-hand 
!    corner of the triangle.
!
!    Output, real(kind=dp) RESULT, the estimated value of the integral.
!
!    Output, real(kind=dp) ABSERR, estimate of the absolute error computed from
!    RESULT and the 3 previous results.
!
!    ?, real(kind=dp) RES3LA(3), the last 3 results.
!
...
!    Input, real(kind=dp) A, B, the limits of integration.
!
!    Input, real(kind=dp) OMEGA, the multiplier of X in the weight function.
!
...
!
  ...
  if ( 5.0e-01*abs(b-a)*domega <= 2.0_dp) then
    numrl2 = 1
    extall = .true.
    rlist2(1) = result
  end if

  if ( 2.5e-01 * abs(b-a) * domega <= 2.0_dp ) then
    extall = .true.
  end if
...
!
    ...
    if ( 2.5e-01*width*domega > 2.0_dp ) then
      cycle
    end if
...
200 continue
  ...
  if ( integr == 2 .and. omega < 0.0_dp ) then
    result = -result
  end if
...
!    Output, real(kind=dp) ABSERR, an estimate of | I - RESULT |.
!
!    Output, real(kind=dp) RESABS, approximation to the integral of the absolute
!    value of F.
!
!    Output, real(kind=dp) RESASC, approximation to the integral | F-I/(B-A) | 
!    over [A,B].
!
!  Local Parameters:
!
...
!
  ...
  if ( resasc /= 0.0_dp.and.abserr /= 0.0_dp ) then
    abserr = resasc*min ( 1.0_dp,(2.0e+02*abserr/resasc)**1.5_dp)
  end if
...
!    which evaluates the integrand function.
!
!    Input, real(kind=dp) BOUN, the finite bound of the original integration range,
!    or zero if INF is 2.
!
...
!
!    Input, real(kind=dp) A, B, the limits of integration, over a subrange of [0,1].
!
!    Output, real(kind=dp) RESULT, the estimated value of the integral.
...
!    value of F.
!
!    Output, real(kind=dp) RESASC, approximation to the integral of the
...
!
  data xgk(1),xgk(2),xgk(3),xgk(4),xgk(5),xgk(6),xgk(7),xgk(8)/ &
       ...
       8.648644233597691e-01,     7.415311855993944e-01, &
       5.860872354676911e-01,     4.058451513773972e-01, &
       2.077849550078985e-01,     0.0000000000000000_dp/
  ...
  data wg(1),wg(2),wg(3),wg(4),wg(5),wg(6),wg(7),wg(8)/ &
       0.0000000000000000_dp,     1.294849661688697e-01, &
       0.0000000000000000_dp,     2.797053914892767e-01, &
       0.0000000000000000_dp,     3.818300505051189e-01, &
       0.0000000000000000_dp,     4.179591836734694e-01/
...
!                       calling program.
!
!    ?, real(kind=dp) P1, P2, P3, P4, parameters in the weight function
!
...
!
  ...
  real(kind=dp) reskh
  real(kind=dp) result
  real(kind=dp), external :: w
  real(kind=dp), dimension ( 4 ) :: wg = (/ &
    1.294849661688697e-01,     2.797053914892767e-01, &
...
!
  data xgk(1),xgk(2),xgk(3),xgk(4),xgk(5),xgk(6),xgk(7),xgk(8)/ &
       ...
       8.648644233597691e-01,     7.415311855993944e-01, &
       5.860872354676911e-01,     4.058451513773972e-01, &
       2.077849550789850e-01,     0.000000000000000_dp/
...
!  Parameters:
!
!    Input, real(kind=dp) ALFA, a parameter in the weight function w(x), ALFA > -1.
!
!    Input, real(kind=dp) BETA, a parameter in the weight function w(x), BETA > -1.
!
!           ri     - real(kind=dp)
...
!
  ...
  real(kind=dp) resasc
  real(kind=dp) reskh
  real(kind=dp) savfun(21)
  real(kind=dp) w10(5)
  real(kind=dp) w21a(5)
  real(kind=dp) w21b(6)
  real(kind=dp) w43a(10)
  real(kind=dp) w43b(12)
  real(kind=dp) w87a(21)
  real(kind=dp) w87b(23)
  real(kind=dp) x1(5)
  real(kind=dp) x2(5)
  real(kind=dp) x3(11)
  real(kind=dp) x4(22)
...
!    largest error.
!
!    Output, real(kind=dp) ERMAX, the NRMAX-th largest error = ELIST(MAXERR).
!
!    Input, real(kind=dp) ELIST(LIMIT), contains the error estimates.
!
...
!  Parameters:
!
!    Input, real(kind=dp) X, the point at which the weight function is evaluated.
!
!    Input, real(kind=dp) C, the location of the singularity.
!
!    Input, real(kind=dp) P2, P3, P4, parameters that are not used.
!
!    Input, integer KP, a parameter that is not used.
!
!    Output, real(kind=dp) QWGTC, the value of the weight function at X.
!
  implicit none
...
!    Input, real(kind=dp) X, the point at which the weight function is evaluated.
!
!    Input, real(kind=dp) OMEGA, the factor multiplying X.
!
!    Input, real(kind=dp) P2, P3, P4, parameters that are not used.
!
...
!    2, W(X) = sin ( OMEGA * X )
!
!    Output, real(kind=dp) QWGTO, the value of the weight function at X.
!
  implicit none
...
!    Input, real(kind=dp) X, the point at which the weight function is evaluated.
!
!    Input, real(kind=dp) A, B, the endpoints of the integration interval.
!
!    Input, real(kind=dp) ALFA, BETA, exponents that occur in the weight function.
!
!    Input, integer INTEGR, specifies which weight function is used:
!    1. W(X) = (X-A)**ALFA * (B-X)**BETA
!    2, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A)
...
!    4, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A) * log(B-X)
!
!    Output, real(kind=dp) QWGTS, the value of the weight function at X.
!
  implicit none
...
!!$      INTEGER  IWR,NP,NP2,NP4
!!$      REAL*8   X(10),W(10),S(128),SN(128)
!!$      INTEGER  L(128),LN(128)
!!$      REAL*8   CTOL,TOL,PTOL,H,XU,XL,A,B,C,D,FCT,SUM,ASUM
!!$      INTEGER  IER,I1,I2,LHN,I,K
!!$!  ****  PRINTED OUTPUT OF PARTIAL RESULTS: SET IWR=1.                  
!!$!      PARAMETER(IWR=0)
!!$!  ****  COEFFICIENTS FOR GAUSS 20-POINT INTEGRATION.                   
!!$      PARAMETER(NP=10,NP2=20,NP4=40)                                         
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)                               
...
!  ****  CORRECTED TOLERANCE.                                           
   ...
   11 FORMAT(/5X,'XL = ',1PD15.8,', XU = ',D15.8,', TOL =',D8.1)      
      ENDIF                                                             
      IER=0                                                             
...
!  ****  ADAPTIVE BIPARTITION SCHEME.                                   
   ...
   12 FORMAT(5X,'N =',I5,', SUM =',1PD19.12,', ERR =',D8.1,', LH =',I3) 
