    ...
    integer, parameter :: npar_atz = 4 !AGS: number of derived parameters per redshift output

    integer, parameter :: max_inipower_params = 10

    real(mcp), parameter :: cl_norm = 1e-10_mcp !units for As
    integer, parameter :: max_derived_parameters = 200    !AGS: increased maximum number of derived par.
    ...
    real(mcp) :: z_default(3) = [0.38_mcp, 0.495_mcp, 0.61_mcp] !AGS
    real(list_prec) :: z_nonlin(1) = 0. !AGS
    ...
    Type TCosmoTheoryParams
        ...
        real(mcp), dimension(:), allocatable :: z_outputs !AGS: add as many z_outputs as power spectra are computed
        ...
        real(mcp) :: power_kmax = 1.3_mcp  !AGS (originally 0.8)
        ...
        logical :: use_growth = .false. !AGS, to use gamma as a free parameter
        logical :: local_lag_g2 = .true. !AGS, assume local-Lagrangian relation to set gamma2
        logical :: local_lag_g3 = .false. !AGS, assume local-Lagrangian relation to set gamma3^-
    ...
    integer, parameter               :: nk_table = 120     !AGS
    type TNLRSD_tables                                     !AGS 
      real(mcp), dimension(nk_table) :: k_table 
      real(mcp), dimension(nk_table) :: dP13_dd_table, dP13_dd_d_table
      real(mcp), dimension(nk_table) :: dP13_dv_table, dP13_dv_d_table
      real(mcp), dimension(nk_table) :: dP13_vv_table, dP13_vv_d_table
      real(mcp), dimension(nk_table) :: P22inv_dd_table, P22inv_dd_d_table
      real(mcp), dimension(nk_table) :: P22inv_dv_table, P22inv_dv_d_table
      real(mcp), dimension(nk_table) :: P22inv_vv_table, P22inv_vv_d_table      
      real(mcp), dimension(nk_table) :: gRPT_root_table, gRPT_root_d_table         
      real(mcp), dimension(nk_table) :: b1b2_table, b1g2_table, b2b2_table         
      real(mcp), dimension(nk_table) :: b2g2_table, g2g2_table, b1g3m_table        
      real(mcp), dimension(nk_table) :: b1b2_d_table, b1g2_d_table, b2b2_d_table   
      real(mcp), dimension(nk_table) :: b2g2_d_table, g2g2_d_table, b1g3m_d_table 
      real(mcp), dimension(nk_table) :: b2_table, g2_table, g3m_table               
      real(mcp), dimension(nk_table) :: b2_d_table, g2_d_table, g3m_d_table        
      real(mcp), dimension(nk_table) :: fmu2v1sqb1sq_table, fmu2v1sqb1sq_d_table   
      real(mcp), dimension(nk_table) :: fmu2v1sqb1b2_table, fmu2v1sqb1b2_d_table   
      real(mcp), dimension(nk_table) :: fmu2v1sqb1g2_table, fmu2v1sqb1g2_d_table   
      real(mcp), dimension(nk_table) :: f2mu2v1v2b1_table, f2mu2v1v2b1_d_table  
      real(mcp), dimension(nk_table) :: f2mu2v1v2b2_table, f2mu2v1v2b2_d_table  
      real(mcp), dimension(nk_table) :: f2mu2v1v2g2_table, f2mu2v1v2g2_d_table  
      real(mcp), dimension(nk_table) :: f2mu2v1sqb1sq_table, f2mu2v1sqb1sq_d_table  
      real(mcp), dimension(nk_table) :: f2mu4v1v2b1_table, f2mu4v1v2b1_d_table  
      real(mcp), dimension(nk_table) :: f2mu4v1v2b2_table, f2mu4v1v2b2_d_table  
      real(mcp), dimension(nk_table) :: f2mu4v1v2g2_table, f2mu4v1v2g2_d_table  
      real(mcp), dimension(nk_table) :: f2mu4v1sqb1sq_table, f2mu4v1sqb1sq_d_table  
      real(mcp), dimension(nk_table) :: f3mu4v2sq_table, f3mu4v2sq_d_table   
      real(mcp), dimension(nk_table) :: f3mu6v2sq_table, f3mu6v2sq_d_table   
      real(mcp), dimension(nk_table) :: f3mu2v1v2b1_table, f3mu2v1v2b1_d_table   
      real(mcp), dimension(nk_table) :: f3mu4v1v2b1_table, f3mu4v1v2b1_d_table   
      real(mcp), dimension(nk_table) :: f3mu6v1v2b1_table, f3mu6v1v2b1_d_table   
      real(mcp), dimension(nk_table) :: f4mu2v2sq_table, f4mu2v2sq_d_table   
      real(mcp), dimension(nk_table) :: f4mu4v2sq_table, f4mu4v2sq_d_table   
      real(mcp), dimension(nk_table) :: f4mu6v2sq_table, f4mu6v2sq_d_table   
      real(mcp), dimension(nk_table) :: f4mu8v2sq_table, f4mu8v2sq_d_table   
    end type TNLRSD_tables
    ...
    call Ini%Read('use_growth',this%use_growth)  !AGS: use gamma as a free parameter
    call Ini%Read('local_lag_g2',this%local_lag_g2)    !AGS: assume logal-Lagrangian relation for gamma2
    call Ini%Read('local_lag_g3',this%local_lag_g3)    !AGS: assume logal-Lagrangian relation for gamma3^-
    ...
    !changed to:
    this%use_LSS = this%get_sigma8 ! AGS: only get_sigma8 required now as z_outputs is defined later
    ...
    if(z_nonlin(1) /= 0.)then !AGS
      !add mean redshift of the sample to exact_z
      call exact_z%AddArrayItems(z_nonlin)
    end if
    !call exact_z%AddArrayItems(CosmoSettings%z_outputs) !AGS: now z_outputs are set later
    ...
    if(full_z%Count<2) then   !AGS
      call full_z%Add(0.1d0)
      call full_z%Add(0.2d0)
      call full_z%Add(0.3d0)
      call full_z%Add(0.4d0)
      call full_z%Add(0.5d0)
      call full_z%Add(0.6d0)
      call full_z%Add(0.7d0)
    end if
    ...
    !AGS: now set z_outputs
    if(exact_z%Count==0) then !AGS
      !use default redshift values
      allocate(CosmoSettings%z_outputs(3)) !AGS: same as redshifts for P(k), but exclude z=0
      CosmoSettings%z_outputs = z_default !AGS: redshifts for derived params
    else
      allocate(CosmoSettings%z_outputs(this%num_power_redshifts-1)) !AGS: same as redshifts for P(k), but exclude z=0
      CosmoSettings%z_outputs = this%power_redshifts(2:this%num_power_redshifts) !AGS: redshifts for deriver params
    end if
    write(*,*)'z_outputs= ',CosmoSettings%z_outputs
